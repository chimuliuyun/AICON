...
def calc_MGV(filepath,weight):
    ...
    Minindex = np.int64(0)
    Maxindex = np.int64(GroupVec.shape[0] - 1)
            ...
            for k in np.arange(2):
                if k == 0: 
                    if Gamma_index[j,k] > Minindex:
                        modebranch_vel[branch_idx,j*2 + k] = [GroupVec[np.int64(index),branch_idx+1] for index in np.arange(Gamma_index[j,k]-5,Gamma_index[j,k])]
                    else:                            #actually, this sentence should never be executed and G point should never be the first point
                        modebranch_vel[branch_idx,j*2 + k] = [GroupVec[np.int64(index),branch_idx+1] for index in np.arange(Gamma_index[j,k]+1,Gamma_index[j,k]+6)]
                        break
                if k == 1: 
    ...
    for branch_idx in np.arange(Bandnum):
        for j in np.arange(len(Gamma)): 
            for k in np.arange(2):
                if k == 0:
                    if Gamma_index[j,k] > Minindex:
                        modebranch_freq[branch_idx,j*2 + k] = [Frequency[np.int64(index),branch_idx+1] for index in np.arange(Gamma_index[j,k]-50,Gamma_index[j,k]+1)]
                    else:
                        modebranch_freq[branch_idx,j*2 + k] = [Frequency[np.int64(index),branch_idx+1] for index in np.arange(Gamma_index[j,k],Gamma_index[j,k]+51)]
                        break
                if k == 1:
                    if Gamma_index[j,k] < Maxindex:
                        modebranch_freq[branch_idx,j*2 + k] = [Frequency[np.int64(index),branch_idx+1] for index in np.arange(Gamma_index[j,k],Gamma_index[j,k]+51)]
