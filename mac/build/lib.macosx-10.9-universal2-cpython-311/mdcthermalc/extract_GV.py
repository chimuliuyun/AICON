    ...
    for eachline in fp1:
        ...
        if len(temp) > 0:
            if keystr5 == temp[0]:        
                ...
                countbands = 0
                postemp = np.array([np.float64(temp[i][:-1]) for i in np.arange(3,6)])
#                print('%f %f %f' % (postemp[0],postemp[1],postemp[2]))
                ...
                GroupVec[countpoints,countbands] = np.float64(temp[-1])
                Frequency[countpoints,countbands] = np.float64(temp[-1])
                countbands = countbands + 1
                if Gammaflag == 1:
                    Gammaflag = 0
                    if np.float64(temp[-1]) not in Gamma:                
                        Gamma.append(np.float64(temp[-1]))
            elif keystr3 in eachline:
                Frequency[countpoints,countbands] = np.float64(temp[-1])
            elif keystr4 in eachline:
                #write velocity value to the rest colume of each row of np .array
                vectemp = np.array([np.float64(temp[i][:-1]) for i in np.arange(2,5)])
                vectemp2 = vectemp**2
