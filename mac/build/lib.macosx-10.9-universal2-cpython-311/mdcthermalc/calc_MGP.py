            ...
            for k in np.arange(2):
                if k == 0:
                    if Gamma_index[j,k] > Minindex:
                        modebranch_grun[branch_idx,j*2 + k] = [Gruneisen[np.int64(index),branch_idx]['grun'] for index in np.arange(Gamma_index[j,k]-50,Gamma_index[j,k])]
                    else:
                        modebranch_grun[branch_idx,j*2 + k] = [Gruneisen[np.int64(index),branch_idx]['grun'] for index in np.arange(Gamma_index[j,k]+1,Gamma_index[j,k]+51)]
                        break
                if k == 1:
